...
const UserShortInfo = ({ seller }) => {
    ...
    const lastActive = useMemo(
        () => (seller?.last_login ? dayjs(seller.last_login).fromNow() : "Faol emas"),
        [seller?.last_login]
    );
        ...
        () => [
            {
                ...
                icon: <SyncOutlined className={cn("text-info", "text-[15px]")} />,
                color: "text-info",
            },
            {
                title: "Muvaffaqiyatli ishlar",
                value: seller?.successful_jobs_count,
                icon: <CheckCircleOutlined className={cn("text-primary", "text-[15px]")} />,
                color: "text-primary",
            },
            {
                title: "Muvaffaqiyatsiz ishlar",
                value: seller?.unsuccessful_jobs_count,
                icon: <CloseCircleOutlined className={cn("text-danger", "text-[15px]")} />,
                color: "text-danger",
            },
            {
                title: "Yuklangan mahsulotlar",
                value: seller?.total_products_count,
                icon: <FileTextOutlined className={cn("text-purple", "text-[15px]")} />,
                color: "text-purple",
            },
            {
                title: "Sotilgan mahsulotlar",
                value: seller?.total_sold_documents,
                icon: <ShoppingOutlined className={cn("text-warning", "text-[15px]")} />,
                color: "text-warning",
            },
        ],
        [seller]
    ...
    const imageSrc = useMemo(() => seller?.image || "/static/img/ozodbek.png", [seller?.image]);
    const imageAlt = useMemo(() => seller?.full_name || "User image", [seller?.full_name]);
    ...
    return (
        <div className={cn("bg-light", "p-3", "shadow", "rounded-xl")}>
            ...
            <Divider size="small" />
            <div className="mt-5">
                <span className={cn("block", "mb-3", "font-semibold", "text-[16px]")}>
                    Statistikalar
                </span>
            ...
            <AuthModal open={authModal} onClose={() => setAuthModal(false)} onSuccess={() => setCreateOrderModal(true)} />
            <CreateOrderModal
