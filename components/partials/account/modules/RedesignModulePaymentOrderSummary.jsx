                    ...
                    ecomerce.cartDataItems.map((item) => (
                        ...
                        >
                            ...
                            <div className={cn("flex", "flex-col", "items-end", "gap-2", "min-w-[90px]")}>
                                ...
                                >
                                    <img src="/static/img/xicon.svg" alt="delete" className="w-4 h-4" />
                                </button>

                                {item.discount === 0 ? (
                                    <p className={cn("font-medium", "text-sm", "md:text-base", "text-gray-800", "m-0")}>
                                        {addPeriodToThousands(item.price)} so'm
                                    </p>
                                ) : (
                                    <div className={cn("flex", "flex-col", "items-end")}>
                                        <del className={cn("text-gray-400", "text-xs", "md:text-sm")}>
                                            {addPeriodToThousands(item.price)} so'm
                                        </del>
                                        <p className={cn("font-semibold", "text-sm", "md:text-base", "text-red-500", "m-0")}>
                ...
                <div className={cn("border-t", "p-4", "bg-gray-50")}>
                    {percentage > 0 && (
                        <div className={cn("flex", "justify-between", "items-center", "mb-2")}>
                            <p className={cn("mb-0", "text-sm")}>Sayt xizmat haqi uchun:</p>
                    ...
                    <div className={cn("flex", "justify-between", "items-center")}>
                        <p className={cn("font-bold", "mb-0", fontSizeClass)}>Jami narx:</p>
                        <p className={cn("font-bold", "mb-0", fontSizeClass)}>{hisob} so'm</p>
                    </div>
                </div>
            )}
        </div>
