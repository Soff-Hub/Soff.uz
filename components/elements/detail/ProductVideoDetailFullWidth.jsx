...
const ProductVideoDetailFullWidth = ({
    ...
    const clickPlaylist = (slug, id) => {
        router.push(
            `/account/checkout-one?type=playlist&slug=${slug}&id=${id}`
        );
    };
    ...
    return (
        <>
            <div className="ps-product--detail mt-5 ">
                <div
                    className="row mb-xl-5 mb-lg-5 mb-0 "
                    ...
                    <div className="col-md-4" style={{ overflowY: 'auto' }}>
                        {data?.playlist_document?.length > 0 && (
                            <div className="col-md-12 p-0 border rounded-3">
                                <div
                                    className="p-3 pt-4 bg-white rounded-3"
                                    style={{
                                        display: 'flex',
                                    ...
                                    <p className="m-0">
                                        {`${
                                            data?.playlist_document?.findIndex(
                                                (el) => el.id === product.id
                                            ) + 1
                                        } / ${data?.playlist_document?.length}`}
                                    </p>
                                </div>
                                    ...
                                    {data?.playlist_document?.map(
                                        (item, ind) => (
                                            <div
                                                ...
                                                {item?.id === product.id ? (
                                                    <i className="fa-solid fa-play fs-5"></i>
                                                ) : (
                                                    ind + 1
                                                )}
                                                <ProductVideoCards
                                ...
                                {data?.price !== 0 &&
                                    !data?.is_purchased_playlist && (
                                        <div
                                            onClick={() =>
                                                clickPlaylist(
                                                    data?.playlist_document?.[0]
                                                        ?.slug,
                                                    data?.id
                                                )
                                            }
                        ...
                        <h4 className="fw-medium fs-3 p-3 bg-body-secondary my-4 rounded-3 text-center">
                            O'xshash videolar
                        </h4>
                        {similar?.length > 0 ? (
                            similar.map((item) => (
                                <div key={item?.id} className="col-md-12 my-4">
                                    <ProductVideoCards
                                        type={'similler'}
                                        product={item}
                                        isPlay={isPlay}
                            ...
                            <div
                                className="d-flex border border-danger rounded-3 justify-content-center align-items-center "
                                style={{
                                    height: '100vh',
                                }}>
                                <p className="text-center fw-bold fs-3 text-danger">
                                    Ma'lumot yo'q
                                </p>
                            </div>
                        )}
