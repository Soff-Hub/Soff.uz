...
const BalanceWithDrawModal = ({ open, onClose }) => {
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: (data) =>
            api.post("seller/application/", data),
        onSuccess: () => {
            message.success("So‘rov muvaffaqiyatli yuborildi!");
            form.resetFields();
            onClose();
            queryClient.invalidateQueries(["getCustomBalance"])
        },
        onError: (error) => {
            const errMsg =
            ...
            <div className={styles.wrapper}>
                ...
                >
                        ...
                        rules={[
                            { required: true, message: "Karta raqamini kiriting" },
                            {
                                validator: (_, value) => {
                                    ...
                                    if (digits.length !== 16) {
                                        return Promise.reject("Karta raqami 16 xonali bo‘lishi kerak");
                                    }
                    ...
                    <Form.Item
                        ...
                        name="amount"
                        rules={[
                            { required: true, message: "Iltimos summani kiriting" },
                            {
                                validator: (_, value) => {
                                    if (!value) return Promise.resolve();
                                    if (value <= 0) return Promise.reject("Summani to‘g‘ri kiriting");
                                    return Promise.resolve();
                                },
                            },
                        ]}
