...
export default function HeaderNotifications({ color }) {
    ...
    const handleRead = async () => {
        try {
            ...
            console.error("Bildirishnomalarni o‘qilgan qilishda xatolik:", error)
        }
    }
            ...
            description: (
                <div>
                    ...
                    <div className={cn("flex", "justify-between", "items-center")}>
                        ...
                        <span onClick={handleRead} className={cn("text-primary", "cursor-pointer", "hover-underline", "transition")}>O'qildi</span>
                    </div>
                </div>
            ),
            icon: (
        ...
        <div>
            ...
            {token ? (
                <Link href={`/account/notification`} style={{ marginRight: '10px' }}>
                    <Badge offset={[-10, 3]} size="small" count={notificationsCount} color='#00a44f'>
                        <a className="header__extra fs-1" style={{ cursor: 'pointer' }}>
