...
const footerMenu = {
    ...
    contactModeration: {
        ...
        items: [
            { contact: "Barcha xizmatlar", url: "/orders/?direction=scientific_work" },
            { contact: "Buyurtma berish", url: "/orders" },
            { contact: "Mahsulotlar", url: "/scientific-resources/all?slug=all" },
            { contact: "Frilanserlar uchun", url: "https://seller.soff.uz/" },
        ],
    },
};
    ...
    return (
        <div className={styles.mainblock}>
            <footer className="bg-transparent container p-lg-0 d-flex justify-content-between align-items-start flex-wrap">
                {/* Asosiy shior */}
                <a href={footerMenu.soff.path} className={styles.footerMainTitle}>
                    Tayyor mahsulotlar va xizmatlar bir joyda
                </a>

                {/* Moderatsiya bo‘limi */}
                <div className="col-xl-3 col-lg-3 col-md-6 col-sm-12 col-12 p-0 mb-5">
                    {footerMenu.contactModeration.items.map((item) => (
                        <a
                            key={item.url}
                            href={item.url}
                            className="d-flex text-white align-items-center gap-3 footer-link fs-4 mb-3"
                        >
                            {item.contact}
                        </a>
                    ))}
                ...
                <div className="col-xl-3 text-white col-lg-3 col-md-6 p-0 col-sm-12 col-12">
                    ...
                    {footerMenu.services.links.map((link, i) =>
                        link.link ? (
                            ...
                            <p key={i} className="text-white d-block fs-4 mb-3">
                                {link.name}
                            </p>
                        )
                    )}
